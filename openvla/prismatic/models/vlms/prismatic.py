...
class AdaLN(nn.Module):  
    def __init__(self, num_features, num_conds):  
        ...
        # 这里我们简单地将条件向量 z 映射到缩放和平移参数上，  
        # 但请注意，这并不是 AdaLN 的典型实现。  
        # 在实践中，你可能需要更复杂的网络来根据 z 生成这些参数。  
...
class DenoisingMLP(nn.Module):  
    def __init__(self, in_channels, out_channels, num_blocks=3, width=1024, num_conds=None):  
        ...
        # self.convert_linear = nn.Linear(4096, width)  
...
class PrismaticVLM(VLM):
    def __init__(
        ...
        **kwargs,
        ...
        if 'DIFFHEAD' in os.environ:
            self.diffuse_action_head = DenoisingMLP(in_channels=1024, out_channels=7, num_blocks=6, width=1024, num_conds=4096)
            self.time_emb = SinusoidalPosEmb(4096)

            # self.noise_scheduler = DDPMScheduler(prediction_type = 'epsilon',)
            # # FOR INFERENCE
            # self.noise_scheduler_eval = DDIMScheduler(prediction_type = 'epsilon',)
    ...
    def gradient_checkpointing_enable(self, gradient_checkpointing_kwargs):
        ...
        self.llm_backbone.enable_gradient_checkpointing()
        pass
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        import os
        if 'NO_PRETRAIN' not in os.environ:

            original_state_dict = model_state_dict["projector"]
            if 'MAST3R' in os.environ:
                # 加载原先模型的参数
                ...
                # 假设原模型的第一层输入特征为 2048
                # 只加载与新模型第一层相匹配的部分
                init_weight = vlm.projector.projector[0].weight.data
                init_weight[:, :2176] = original_state_dict['projector.0.weight']
                original_state_dict['projector.0.weight'] = init_weight
                # vlm.projector.projector[0].weight.data[:, :2048] = original_state_dict['projector.0.weight']

            vlm.projector.load_state_dict(original_state_dict)
            vlm.llm_backbone.load_state_dict(model_state_dict["llm_backbone"])
            if "vision_backbone" in model_state_dict.keys():
                vlm.vision_backbone.load_state_dict(model_state_dict["vision_backbone"])
            else:
                print("not load vision backbone from vlm!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!", pretrained_checkpoint, flush=True)
        else:
            print('not load pretrainingggggggggggggggggggg')
        # Freeze Weights
        if freeze_weights:
    ...
    def freeze_backbones(self, stage: str, freeze_mast3r=False) -> None:
        """
        ...
        if stage == "align":
            ...
            self.projector.requires_grad_(True)
            if freeze_mast3r:
                self.vision_backbone.mast3d_featurizer.requires_grad_(False)
    ...
    def forward(
        ...
        with torch.set_grad_enabled(self.vision_backbone_requires_grad):
            if isinstance(pixel_values, dict):
                if len(pixel_values[list(pixel_values.keys())[0]].shape) == 5:
                    window_size = pixel_values[list(pixel_values.keys())[0]].shape[1]
                    patch_features = self.vision_backbone({k: pixel_values[k][multimodal_indices].flatten(0, 1) for k in pixel_values})
                    # import ipdb;ipdb.set_trace()
                    b_ = patch_features.shape[0]
                    # 2 is the obseravation size 
                    patch_features = patch_features.view(b_//window_size, window_size*patch_features.shape[1], -1)
                else:
                    # 4
                    patch_features = self.vision_backbone({k: pixel_values[k][multimodal_indices] for k in pixel_values})

                    self.vision_backbone({k: pixel_values[k][multimodal_indices] for k in pixel_values})
            else:
                # print(pixel_values[multimodal_indices].shape)
                if len(pixel_values.shape) == 5:
                    window_size = pixel_values.shape[1]
                    patch_features = self.vision_backbone(pixel_values[multimodal_indices].flatten(0, 1))
                    # import ipdb;ipdb.set_trace()
                    b_ = patch_features.shape[0]
                    patch_features = patch_features.view(b_//window_size, window_size*patch_features.shape[1], -1)
                else:
