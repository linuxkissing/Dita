...
class DinoSigLIPViTBackbone(VisionBackbone):
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int = 224) -> None:
        ...
        # you can put the path to a local checkpoint in model_name if neede
        self.use_mast3r = 0
        ...
        if self.use_mast3r:
            import sys
            sys.path.append('/xxx/xxx/Anonymous/Code/mast3r')
            from mast3r.model import AsymmetricMASt3R
            self.mast3d_featurizer = AsymmetricMASt3R.from_pretrained(model_name)
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # encode the two images --> B,S,D
            b = len(pixel_values["siglip"])
            shape = pixel_values["siglip"].shape
            # import ipdb;ipdb.set_trace()
            # torch.Size([64, 3, 224, 224])
            view = {'img': pixel_values["siglip"].view(b//2, 2, shape[1], shape[2], shape[3])[:,0,:,:],
                    'idx': 0,
                    'instance':'0'}
            view1 = {'img': pixel_values["siglip"].view(b//2, 2, shape[1], shape[2], shape[3])[:,1,:,:],
                    'idx': 0,
                    'instance':'0'}
            if self.use_mast3r == 2:
                (shape1, shape2), (feat1, feat2), (pos1, pos2) = self.mast3d_featurizer._encode_symmetrized(view, view1)
                dec1, dec2 = self.mast3d_featurizer._decoder(feat1, pos1, feat2, pos2)
                # import ipdb;ipdb.set_trace()
                mast3d_patches1 = torch.cat([dec1[-1], torch.zeros([len(dec1[-1]), 256-196, 768]).to(siglip_patches.device)], dim=1)
                # mast3d_patches2 = torch.cat([dec2[-1], torch.zeros([len(dec2[-1]), 256-196, 768]).to(siglip_patches.device)], dim=1)
                # mast3d_patches = torch.cat([mast3d_patches1, mast3d_patches2],  dim=0)
                mast3d_patches = mast3d_patches1
            # 768
            else:
                mast3d_patches, ppp, ttt = self.mast3d_featurizer._encode_image(pixel_values["siglip"], (224,224))
                # 256 but got size 196
                mast3d_patches = torch.cat([mast3d_patches, torch.zeros([len(dino_patches), 256-196, 1024]).to(mast3d_patches.device)], dim=1)
            return torch.cat([dino_patches, siglip_patches, mast3d_patches], dim=2)
        else:
    ...
    def embed_dim(self) -> int:
        if self.use_mast3r == 1:
            return self.dino_featurizer.embed_dim + self.siglip_featurizer.embed_dim + self.dino_featurizer.embed_dim
        elif self.use_mast3r == 2:
            return self.dino_featurizer.embed_dim + self.siglip_featurizer.embed_dim + 768
        else:
            return 1024 + self.siglip_featurizer.embed_dim
