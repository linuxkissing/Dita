...
# from rt1_pytorch.openx_utils.robot_utils import joints_to_pose_batch_tf
...
def get_builder(name, data_dir):
    if name == 'bridge_state':
        name = 'bridge_orig'
    if name == "droid_cam":
        name = "droid"
    if name.endswith('_state'):
        name = name[:-len('_state')]
    if name in ['viola', 'dobbe', 'cmu_stretch', 'taco_play', 'jaco_play', 'berkeley_autolab_ur5', 'fractal20220817_data',  'berkeley_cable_routing', 
                ...
                'taco_play', 'toto', 'ucsd_kitchen_dataset_converted_externally_to_rlds', 'utaustin_mutex', 'kuka', 'language_table',
                'austin_sailor_dataset_converted_externally_to_rlds']:
    #if False:
    # 'robo_set', 
        print(name, data_dir, 'resize ================================', flush=True)
        if data_dir.__contains__('s3://openx'):
            builder = tfds.builder(name, data_dir=data_dir.replace('s3://openx', 's3://openx/resized/'))
        elif data_dir.__contains__('s3:/openx'):
            builder = tfds.builder(name, data_dir=data_dir.replace('s3:/openx', 's3://openx/resized/'))
        else:
            print('use orig data')
            builder = tfds.builder(name, data_dir=data_dir.replace('s3:/', 's3://') if data_dir.startswith('s3:/') else data_dir)
    elif name in ['robo_set']:
        # //openx/resized1/robo_set/
        print('s3://openx/resized1/', data_dir.replace('s3:/openx', 's3://openx/resized1/'))
        builder = tfds.builder(name, data_dir=data_dir.replace('s3:/openx', 's3://openx/resized1/'))
    else:
        print(data_dir.replace('s3:/', 's3://') if data_dir.startswith('s3:/') else data_dir)
        builder = tfds.builder(name, data_dir=data_dir.replace('s3:/', 's3://') if data_dir.startswith('s3:/') else data_dir)
        # bridge_orig
    return builder
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        # TODO remove depth image to reduce memory consume
        ...
        if state_obs_keys:
            ...
            # new_obs["proprio"] = old_obs['proprio']
        ...
        if language_key is not None:
            ...
            # task["language_instruction"] = tf.expand_dims(traj.pop(language_key), axis=0)
        ...
        traj = {
            ...
            # "dataset_name": tf.repeat(tf.expand_dims(name, axis=0), traj_len),
            "dataset_name": tf.repeat(name, traj_len),
        }
        
        # print(traj.keys(), )
        # tf.print(traj['task'], [tf.shape(traj['task']['language_instruction']), tf.shape(traj['action']), tf.shape(traj['observation']['image_primary']), ])
    ...
    while True:
        try:
            builder = get_builder(name, data_dir)
    ...
    if isinstance(dataset_statistics, str):
        ...
        if max_action is not None:
            def func1(x):
                ...
                return tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action)
            # dataset = dataset.filter(lambda x: tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action))
            full_dataset = full_dataset.filter(func1)
            pass
        ...
        dataset_statistics = get_dataset_statistics(
            ...
            dataset_name=name if max_action is None else name + str(max_action),
        )
    dataset_statistics = tree_map(np.array, dataset_statistics)
    print("dataset_statistics",dataset_statistics, flush=True)
    ...
    #     print(item['traj_index'], item['task']['language_instruction'])
    #     break
    # import ipdb;ipdb.set_trace()
    if max_action is not None:
        def func1(x):
            # tf.print(max_action, tf.math.abs(x["action"][...,:6]), tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action))
            return tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action)
        # dataset = dataset.filter(lambda x: tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action))
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    if skip_unlabeled:
        ...
        # def func1(x):
        #     tf.print(x["task"]["language_instruction"], x["task"]["language_instruction"][0], tf.math.reduce_any(x["task"]["language_instruction"] != ""))
        #     # return tf.math.reduce_any(x["task"]["language_instruction"] != "")
        #     return tf.math.reduce_any(x["task"]["language_instruction"] != "")
    ...
    if train and task_augment_strategy is not None:
        ...
        print('task_augment_strategy=======================', task_augment_strategy)
        # import ipdb;ipdb.set_trace()
        dataset = dataset.traj_map(
            partial(
    ...
    if max_action is not None:
        def func1(x):
            # tf.print(max_action, tf.shape(x['action']), tf.math.abs(x["action"][...,:6]), tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action,axis=[1,2]))
            return tf.math.reduce_any(tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action,axis=[1,2]))
        # dataset = dataset.filter(lambda x: tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action))
        dataset = dataset.filter(func1)
        ...
        # dataset = dataset.traj_map(
        #         partial(traj_transforms.filter_by_max_action, max_action=max_action),
        #         num_parallel_calls,
        #     )
    # print('max_proprio', max_proprio)
    if max_proprio is not None:
        def func1(x):

            return tf.math.reduce_any(tf.math.reduce_all(tf.math.abs(x["proprio"][...,:6]) != 0,axis=[1,2]))
        # dataset = dataset.filter(lambda x: tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action))
        dataset = dataset.filter(func1)
        pass
    if train and subsample_length is not None:
        ...
        elif subsample_type == 'goal':
            dataset = dataset.traj_map(
                partial(traj_transforms.subsample_withgoal, subsample_length=subsample_length),
                num_parallel_calls,
            )
        elif subsample_type == 'goalprop':
            dataset = dataset.traj_map(
                partial(traj_transforms.subsample_withgoalprop, subsample_length=subsample_length),
                num_parallel_calls,
            )
...
def apply_frame_transforms(
    ...
    *,
    ...
    dataset = dataset.frame_map(
        partial(
            apply_obs_transform,
            partial(obs_transforms.decode_and_resize, resize_size=resize_size, depth_resize_size=depth_resize_size, center_crop= center_crop),
        ),
        num_parallel_calls,
    )
    print('apply_obs_transform, end')
    if train:
...
def make_interleaved_dataset(
    ...
    *,
    ...
    for dataset_kwargs in dataset_kwargs_list:
        if dataset_kwargs['name'] in ['gso_6dof', 'gso_6dof1']:
            # 6DoF Pose estimation
            # builder = tfds.builder(dataset_kwargs['name'], data_dir='s3://openx')
        
            # dataset = builder.as_dataset(split='train')
            # for item in dataset.as_numpy_iterator():
            #     pass
            dataset_sizes.append(217233)
        elif dataset_kwargs['name'] in ['objaverse_6dof', 'gso_6dof1']:
            # 6DoF Pose estimation
            # builder = tfds.builder(dataset_kwargs['name'], data_dir='s3://openx')
            ...
            datasets_stat.append(tmp_dataset)
    ...
    try:
        ...
        print(dataset_sizes, sample_weights, primary_dataset_indices, 'len')
        raise e
    ...
    print('threads:', traj_transform_threads, traj_read_threads, threads_per_dataset, reads_per_dataset)
    ...
    for dataset_kwargs, threads, reads in zip(
        ...
        dataset_kwargs['clamp_value'] = clamp_value
        print('clamp_value1', clamp_value, data_kwargs['clamp_value'])
        ...
        print('dataset_frame_transform_kwargs:', dataset_frame_transform_kwargs)
